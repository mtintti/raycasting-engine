...
void Renderer::DebugDrawLine(float x1, float y1, float x2, float y2) {
    glUseProgram(0);
    glBindVertexArray(0);
    glBegin(GL_LINES);
        glColor3f(0, 1, 0);
...
void Renderer::DebugDrawLinepast(float x1, float y1, float x2, float y2) {
    glUseProgram(0);
    glBindVertexArray(0);
    //std::cout << "\n debug drawline " << x1 << " "<< y1 <<" next -> "<< x2 << " " << y2;
    glBegin(GL_LINES);
        glColor3f(0, 1, 1);
        glVertex2f(x1, y1);
        glVertex2f(x2, y2);
